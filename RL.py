...
class Environment(MissionWrapper, gym.Env):
    ...
    def reset(self, delay_start=30, max_iter=1000, regen=False, eval=False, **kwargs):
        """
    ...
    def step(self, action):
        ...
        ones = np.where(action == 1)[0]
        if len(ones) > 1:
            chosen_index = np.random.choice(ones)
            new_action = np.zeros_like(action)
            new_action[chosen_index] = 1
            action = new_action
        else:
            chosen_index = -1
        # (Optional: If no 1 is present, you may want to handle that case.)
    ...
    def calculate_reward(self, state, action, done, dupe_reward=False):
        ...
        dist_mult = 100
        path_confidence_weight = 3
        radius_mult = 1
        bomb_mult = 0
        action_mult = 1

        dupe_reward_val = -5000
        ...
        distance_reward = (abs(previous_ugv_distance - ugv_distance)
                        if self.iter <= self.delay_start and not done
        ...
        if self.iter > self.delay_start and hasattr(self, 'cumulative_ugv_weights'):
        ...
        # - If avg_weight < 20: a large bonus.
        if avg_weight > 30:
            path_confidence_reward = -20 * (avg_weight - 30)
        elif 25 <= avg_weight <= 30:
            path_confidence_reward = 10 * (np.exp((30 - avg_weight) / 10) - 1)
        else:  # avg_weight < 20
            path_confidence_reward = 1000

        reward += path_confidence_reward * path_confidence_weight
        if done:
            self.current_episode_metrics['path_confidence'] = avg_weight

        return reward

    def calculate_reward_old(self, state, action, done):
        """
        Compute the reward for the current state and action.
        The reward now includes a bonus for scanning cells with high confidence,
        i.e. when the predictions are further from 0.5.
        """
        ...
        # Reward based on UGV distance improvement

        #### REMOVED ABS HERE, MIGHT NEED TO ADD BACK
        distance_reward = abs(previous_ugv_distance - ugv_distance) if self.iter <= self.delay_start and not done else abs((previous_ugv_distance - ugv_distance) - self.BASE_WEIGHT)

        # Action reward (e.g., penalty if action is not one-hot)
        action_reward = -1 * (action.sum() - 1) if action.sum() != 1 else 100
        
        # Radius reward based on the scan counts
        ai_count = np.count_nonzero(state['UGV_STATE']['ai_estimates'] is not None)
        human_count = np.count_nonzero(state['UGV_STATE']['human_estimates'] is not None)
        radius_reward = (ai_count + human_count)

        # Bomb traversed penalty
        traversal_status = state['UGV_STATE']['traversal_status']
        last_move_idx = np.where(state['UGV_STATE']['last_move'] == 1)[0]
        if len(last_move_idx) > 0 and traversal_status[last_move_idx[0]] == 2:
            bomb_reward = 0
        else:
            bomb_reward = 0
        # ['edges', 'metadata', 'traversal_status', 'ai_estimates', 'human_estimates', 
        ...
        # Sum all components (note the subtraction of the action penalty)
        reward = (dist_mult * distance_reward +
                radius_mult * radius_reward +
                bomb_mult * bomb_reward +
                action_mult * action_reward)
        ...
        path = nx.shortest_path(self.network, source=self.ugv_node, target=self.GOAL, weight='weight')
        if len(path) > 2:
            edge_weights = np.array([self.network[u][v]['weight'] for u, v in zip(path[:-1], path[1:])])
            avg_weight = edge_weights.mean()

            if avg_weight < 25:
                reward += SAFE_PATH_BONUS
                print("SAFE")
...
def train(timestamp = None):
    ...
    if initial_metrics_df['mines_encountered'].values[0] != 5:
        model = None
    ...
    eval_interval = 100  # number of timesteps per training chunk
    timesteps_trained = 0
    ...
    while timesteps_trained < total_timesteps:
        ...
        env.save(os.path.join(saves_folder, f"out_{timesteps_trained}.csv"))
        ...
        if metrics_df.loc[metrics_df.index[-1], "mines_encountered"] == 0:
...
def main(maxSteps=1000):
    ...
    env = Environment()
    env.reset(delay_start=0, eval=True)
    env.iter = 0
    done = False
    total_mines = 0
    ...
    while not done:
        ...
        print(env.last_ugv_edge, env.last_uav_edge)
        ...
        #['edges', 'metadata', 'traversal_status', 'ai_estimates', 
        # 'human_estimates', 'path_array', 'move_array', 
        # 'last_move', 'num_connections', 'uav_distance_to_goal', 
        # 'ugv_distance_to_goal', 'ugv_path_array']
    print(total_mines)
